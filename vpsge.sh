...
# 移除或注释掉 set -e，避免交互式菜单在某一步命令失败时直接导致脚本崩溃退出
# set -e
...
run_all() {
    ...
    # detect_distro 已在全局入口执行过，此处移除冗余调用
    bootstrap_packages
...
main_menu() {
    while true; do
        ...
        read -rp "请选择: " opt
        case $opt in
            # 移除了所有子菜单中冗余的 detect_distro，因为入口处已经完成检测
            1) menu_basic ;;
            2) menu_ssl ;;
...
# ────────────────────────────────────────────────────────────────
install_self() {
    local target="/usr/local/bin/vpsge"
    local tmp_file="/tmp/vpsge.sh"

    # 如果当前已经在目标路径运行，则跳过
    [[ "$0" == "$target" ]] && return 0

    # 先下载到临时文件，确保下载完整后再覆盖，防止网络中断导致脚本损坏
    if curl -fsSL --connect-timeout 10 "${VPSGE_BASE_URL}/vpsge.sh" -o "$tmp_file" 2>/dev/null || \
       wget -qO "$tmp_file" "${VPSGE_BASE_URL}/vpsge.sh" 2>/dev/null; then
        
        chmod 755 "$tmp_file"
        mv -f "$tmp_file" "$target"
        
        # 兼容处理：建立软链接到 /usr/bin 以防旧版路径调用失败
        ln -sf "$target" /usr/bin/vpsge
    fi

    # 安全检查依赖的函数是否存在，再输出成功信息
    if type is_cmd_exist >/dev/null 2>&1 && is_cmd_exist vpsge; then
