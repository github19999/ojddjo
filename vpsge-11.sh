...
VPSGE_REMOTE_URL="${VPSGE_BASE_URL}/vpsge-11.sh"
...
MODULES=(
    mod01_globals-1.sh
    mod02_basic_security-1.sh
    mod03_ssl-1.sh
    mod04_install_service-1.sh
    mod05_singbox_config-1.sh
    mod06_service_mgmt-1.sh
    mod07_gen_links-1.sh
)
...
# ────────────────────────────────────────────────────────────────
#  安装 vpsge 快捷命令
#  将 vpsge-11.sh 复制到 /usr/bin/vpsge，此后直接输入 vpsge 即可启动
# ────────────────────────────────────────────────────────────────
install_self() {
    ...
    if curl -fsSL --connect-timeout 10 "https://raw.githubusercontent.com/github19999/ojddjo/main/vpsge-11.sh" -o "$target" 2>/dev/null || \
       wget -qO "$target" "https://raw.githubusercontent.com/github19999/ojddjo/main/vpsge-11.sh" 2>/dev/null; then
        chmod 755 "$target"
    fi
